...
const RED_DARK = "#dc2626"; // red-600

// ── Angle helpers ─────────────────────────────────────────────────────────────

// Keep accumulated rotation as a continuous value (no 0-360 wrapping)
// so CSS transition always takes the shortest visual path.
function shortestDelta(from, to) {
  let d = ((to - from) % 360 + 540) % 360 - 180; // always in [-180, 180]
  return d;
}
...
function createMarkerElements(heading = 0) {
  ...
  inner.innerHTML = `
    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 36 36">
      <path
        d="M 4.48 14.38 A 14 14 0 0 1 31.52 14.38"
        fill="none"
        stroke="${RED}"
        stroke-width="7"
...
export function useStreetView(mapRef, mapZoom) {
  ...
  const accumulatedAngle = useRef(0); // continuous angle, never wraps
  ...
  const onPanoChange = useCallback(({ heading, lat, lng }) => {
    if (innerElRef.current) {
      // Accumulate continuous angle to avoid wrap-around snap
  ...
  useEffect(() => {
    ...
    const handleClick = async (e) => {
      ...
      try {
        ...
        if (meta.status !== "OK") {
          setPanel({ open: true, location: null, heading: 0, loading: false, error: "No street-level imagery found nearby." });
          return;
        }
        ...
        accumulatedAngle.current = heading; // seed with initial heading
